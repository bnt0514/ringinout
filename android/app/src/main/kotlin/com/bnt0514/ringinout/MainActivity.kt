...
class MainActivity : FlutterActivity() {
    ...
    // ✅ 뒤로가기 버튼 - Flutter의 PopScope/Navigator가 처리하도록 위임
    // Flutter 쪽 MainNavigationPage의 PopScope에서:
    //   - 다른 탭 → 알람 탭으로 이동
    //   - 알람 탭 → moveTaskToBack (백그라운드)
    // 따라서 여기서 직접 moveTaskToBack 하지 않음
    @Deprecated("Deprecated in Java")
    override fun onBackPressed() {
        super.onBackPressed() // Flutter 엔진에 back 이벤트 전달
    }
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(
                ...
                .setMethodCallHandler { call, result ->
                    when (call.method) {
                        "moveTaskToBack" -> {
                            Log.d("MainActivity", "📦 Flutter 요청 - 앱을 백그라운드로 이동")
                            moveTaskToBack(true)
                            result.success(true)
                        }
                        else -> result.notImplemented()
