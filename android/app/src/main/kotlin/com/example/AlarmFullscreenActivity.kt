...
class AlarmFullscreenActivity : Activity() {
    ...
    private fun scheduleSnooze(minutes: Int) {
        ...
        SnoozeScheduler.scheduleSnooze(this, alarmId, alarmTitle, minutes)
    ...
    private fun dismissAlarm() {
        ...
        val prefs = getSharedPreferences("ringinout", Context.MODE_PRIVATE)
        prefs.edit().apply {
            remove("trigger_count_$alarmId")
            // ✅ 알람 비활성화 플래그 설정 (Flutter에서 확인)
            putBoolean("alarm_disabled_$alarmId", true)
            apply()
        }
